...
def home(request):
    blog_info = introduce_channel.objects.all() 
...
def new(request): 
    if request.method == 'POST' : #글을 작성한 후 저장 버튼을 눌렀을 때 = 파라미터로 받은게 있을때
        ...
        form = YoutubeChannelForm() #글을 입력받기 위한 빈 form을 불러온다  
        return render(request,'new.html', {'form' : form})

def edit(request,id):
    update_blog = get_object_or_404(introduce_channel, pk = id)
    if request.method == 'POST': #update #글을 수정하고 수정버튼을 눌렀을 때
        form = YoutubeChannelForm(request.POST, request.FILES, instance = update_blog)
        #현재 post에 가져온 정보를 form에 담는다
        if form.is_valid():
            update_blog = form.save(commit = False)
            update_blog.date = timezone.now()
            update_blog.save()
            return redirect('/favorite/detail/' + str(id))#글이 저장되었으면 detail페이지  
        return redirect('home') #가져온정보가 유효하지 않을 때 home으로 가게
    else: #Get방식
        form = YoutubeChannelForm(instance = update_blog)
        return render(request, 'edit.html', {'form' : form})
        #수정된 정보를 edit.html에 보내준다.

def delete(request,id) :
    delete = introduce_channel.objects.get(id = id)
    delete.delete()
    return redirect('home')      

def search(request):
    blogs = introduce_channel.objects.all().order_by('-id')
    #모든 객체를 id의 역순으로 담는다

    find = request.POST.get('find', "")

    if find:
        blogs = blogs.filter(channel__icontains=find)
        #Post로 넘어온 값 비교
